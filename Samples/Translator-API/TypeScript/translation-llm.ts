...
async function translateText(text: string, from: string, to: string, model: string): Promise<TranslationResult[]> {
    ...
    const body = [
        {
            text,
            language: from,
            targets: [{ language: to, deploymentName: model }],
        },
    ];
...
export async function main(): Promise<void> {
    ...
    const translations = await translateText(textToTranslate, fromLanguage, targetLanguage, model);
