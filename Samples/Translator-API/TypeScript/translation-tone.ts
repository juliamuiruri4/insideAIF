...
async function translateText(text: string, from: string, to: string, model: string, tone: string): Promise<TranslationResult[]> {
    ...
    const body = [
        {
            text,
            language: from,
            targets: [{ language: to, deploymentName: model, tone: tone }],
        },
    ];
...
export async function main(): Promise<void> {
    const textToTranslate = "Can you send me the details?";
    const fromLanguage = 'en';
    const targetLanguage = 'fr';
    const model = 'your-gpt-model-deployment';
    const tone = 'informal';
    
    const translations = await translateText(textToTranslate, fromLanguage, targetLanguage, model, tone);
